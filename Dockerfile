...
# Capability-specific Kali runtime for SaturnX MCP.
#
...
LABEL maintainer="SaturnX MCP Server"
LABEL description="Selective tooling image for the SaturnX offensive security MCP server"
...
ARG TARGETPLATFORM
ARG SATURNX_TARGET_PLATFORM=${TARGETPLATFORM}

# Core is mandatory. Optional apt packages are added only for selected bundles.
RUN --mount=type=secret,id=saturnx_build_ca \
    set -eux; \
    has_cap() { [ "$SATURNX_CAPABILITIES" = all ] || case ",$SATURNX_CAPABILITIES," in *",$1,"*) return 0 ;; *) return 1 ;; esac; }; \
    case "$SATURNX_TARGET_PLATFORM" in linux/amd64|linux/arm64) ;; *) echo "unsupported SaturnX image platform: $SATURNX_TARGET_PLATFORM" >&2; exit 1 ;; esac; \
    test -z "$TARGETPLATFORM" || test "$TARGETPLATFORM" = "$SATURNX_TARGET_PLATFORM"; \
    case "$(dpkg --print-architecture)" in amd64) test "$SATURNX_TARGET_PLATFORM" = linux/amd64 ;; arm64) test "$SATURNX_TARGET_PLATFORM" = linux/arm64 ;; *) exit 1 ;; esac; \
    ca_bundle=/run/secrets/saturnx_build_ca; \
    if [ -n "$SATURNX_BUILD_CA_SHA256" ]; then test -f "$ca_bundle"; fi; \
    if [ -f "$ca_bundle" ]; then \
        test -n "$SATURNX_BUILD_CA_SHA256"; \
        test "$(wc -c < "$ca_bundle")" -le 1048576; \
        ! grep -Eq -- '-----BEGIN ([^-]*PRIVATE KEY|OPENSSH PRIVATE KEY)-----' "$ca_bundle"; \
        mkdir -p /usr/local/share/ca-certificates/saturnx; \
        awk 'BEGIN { inside=0; count=0; invalid=0 } /-----BEGIN CERTIFICATE-----/ { if (inside) invalid=1; inside=1; count++; file=sprintf("/usr/local/share/ca-certificates/saturnx/build-ca-%03d.crt", count) } inside && $0 !~ /-----BEGIN CERTIFICATE-----/ && $0 !~ /-----END CERTIFICATE-----/ && $0 !~ /^[A-Za-z0-9+\/=[:space:]]+$/ { invalid=1 } inside { print > file } /-----END CERTIFICATE-----/ { if (!inside) invalid=1; close(file); file=""; inside=0; next } !inside && $0 !~ /^[[:space:]]*$/ && $0 !~ /^[[:space:]]*#/ { invalid=1 } END { if (inside || count == 0 || invalid) exit 1 }' "$ca_bundle"; \
        normalized_ca=/tmp/saturnx-build-ca.pem; : > "$normalized_ca"; \
        for certificate in /usr/local/share/ca-certificates/saturnx/build-ca-*.crt; do cat "$certificate" >> "$normalized_ca"; done; \
        printf '%s  %s\n' "$SATURNX_BUILD_CA_SHA256" "$normalized_ca" | sha256sum -c -; \
    ...
    printf '%s\n' 'Acquire::https::CaInfo "/etc/ssl/certs/ca-certificates.crt";' > /etc/apt/apt.conf.d/79-saturnx-ca; \
    printf '%s\n' 'Acquire::Retries "5";' 'Acquire::http::Timeout "60";' 'Acquire::https::Timeout "60";' > /etc/apt/apt.conf.d/80-saturnx-retries; \
...
RUN set -eux; \
    has_cap() { [ "$SATURNX_CAPABILITIES" = all ] || case ",$SATURNX_CAPABILITIES," in *",$1,"*) return 0 ;; *) return 1 ;; esac; }; \
    cleanup_manifest=/opt/saturnx-build-helpers.txt; : > "$cleanup_manifest"; \
    safe_purge() { \
      helper="$1"; \
...
RUN set -eux; \
    ...
    test -n "$SATURNX_CAPABILITY_MANIFEST_SHA256"; \
    spec=/opt/saturnx-capability-manifest.spec; printf 'capabilities=%s\n' "$SATURNX_CAPABILITIES" > "$spec"; for tool in $required; do printf 'binary=%s\n' "$tool" >> "$spec"; done; \
    printf '%s  %s\n' "$SATURNX_CAPABILITY_MANIFEST_SHA256" "$spec" | sha256sum -c -; \
    printf 'capabilities=%s\nplatform=%s\napt_suite=kali-last-snapshot\nmanifest_sha256=%s\n' "$SATURNX_CAPABILITIES" "$SATURNX_TARGET_PLATFORM" "$SATURNX_CAPABILITY_MANIFEST_SHA256" > /opt/saturnx-capabilities.txt; \
    cat /opt/saturnx-build-helpers.txt >> /opt/saturnx-capabilities.txt; \
    for tool in $required; do path="$(command -v "$tool")"; printf '%s=%s\n' "$tool" "$path" >> /opt/saturnx-capabilities.txt; done; \
    if has_cap browser; then \
...
LABEL saturnx.cloakbrowser.version="${CLOAKBROWSER_PY_VERSION}"
LABEL saturnx.cloakbrowser.sha256="${CLOAKBROWSER_WHEEL_SHA256}"
LABEL saturnx.base.repository="kalilinux/kali-last-release"
LABEL saturnx.base.digest="sha256:01a402ec78a2b3bd86394f34f8c3d6adefe3c593ae259ac0779c4d1f971c8ff5"
LABEL saturnx.apt.suite="kali-last-snapshot"
LABEL saturnx.platform="${SATURNX_TARGET_PLATFORM}"
LABEL saturnx.capability_manifest_sha256="${SATURNX_CAPABILITY_MANIFEST_SHA256}"
