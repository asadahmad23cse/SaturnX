...
SERVER_INSTRUCTIONS = """
SaturnX runs authorized security tools in a managed Kali container. Confirm
...
TOOL_DESCRIPTIONS = {
    ...
    "nmap_write_nse_script": _desc(
        ...
        "Pass name without path separators; SaturnX sanitizes it and writes /opt/workspace/nmap-scripts/<name>.nse. Use nmap_run_nse_script to execute that managed artifact. Include NSEDoc metadata and correct categories in content.",
    ...
    "system_start_new_session": _desc(
        "Start a fresh SaturnX container session with a clean mounted workspace.",
    ...
    "system_list_sessions": _desc(
        "List SaturnX session workspaces on the host.",
    ...
    "system_stop_container": _desc(
        "Stop and remove the current SaturnX container while preserving workspace files.",
    ...
    "ctf_steghide": _desc(
        ...
        "Pass filepath and optional passphrase. Without a passphrase SaturnX passes an empty one to avoid an interactive prompt. extra_args can tune steghide behavior.",
...
def backend_unavailable(tool: str, message: str, *, next_steps: Any = None) -> dict:
    return usage_error(
        ...
        "backend_unavailable",
        message,
        next_steps=next_steps or "Start the required backend service or run SaturnX with that backend enabled.",
    )
