"""Interprocess coordination locks used by SaturnX runtime processes."""
...
class InstanceLockError(RuntimeError):
    """Raised when another SaturnX server already owns the checkout lock."""


class SaturnXInstanceLock:
    """Small stdlib-only exclusive file lock scoped to one project checkout."""

    def __init__(self, project_root: Path) -> None:
        self.project_root = project_root.resolve()
        self.lock_path = self.project_root / ".saturnx.lock"
    ...
    def _busy_message(self, owner: dict) -> str:
        ...
        return (
            "Another SaturnX MCP server is already running for this checkout.\n"
            f"Project: {self.project_root}\n"
            f"Owner PID: {pid}\n"
            f"Owner command: {command}\n"
            "Close that Codex/MCP session, or stop the owning process, then start SaturnX again."
        )
...
def _runtime_state_root() -> Path:
    ...
    return base / "saturnx-mcp" / "runtime"


class SaturnXPortAllocationLock:
