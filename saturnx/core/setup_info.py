...
from saturnx.core.config_io import (
...
from saturnx.core.wordlists import WORDLIST_FILES, WORDLIST_SOURCES
...
def capture_surface() -> dict[str, Any]:
    ...
    try:
        ...
        from saturnx.resources.post_exploitation import (
...
def validate_skill_routing(
    ...
    skill_root = Path(source) / "skills" / "saturnx-mcp"
...
def setup_state_locations(project_dir: Path) -> dict[str, str]:
    ...
    return {
        "user": str((root / "saturnx-mcp" / "install.json").resolve()),
        "project": str((project_dir / ".saturnx" / "install.json").resolve()),
    }
...
def _python_environment_facts(root: Path) -> dict[str, Any]:
    ...
    launcher_name = "saturnx.exe" if os.name == "nt" else "saturnx"
...
def _failure_diagnostics() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "retryable": False,
            "resolution": "restore the prior SaturnX entry and validate the active client only",
        },
        {
        ...
        },
        {
            ...
            "retryable": False,
            "resolution": "correct the reported runtime bootstrap defect and restart SaturnX",
        },
        {
            "code": "stale_container_reclaimed",
            "retryable": True,
            "resolution": "retry after SaturnX confirms every owned port was released",
        },
        {
...
def setup_information(
    ...
    *,
    ...
    if requested is None:
        requested = os.getenv("SATURNX_INSTALLED_CAPABILITIES")
    ...
    ca_sha256 = os.getenv("SATURNX_BUILD_CA_SHA256", "").strip().lower()
    if not ca_sha256:
    ...
    if build_ca_bundle is not None:
        ...
        raise ValueError("SATURNX_BUILD_CA_SHA256 must be a SHA-256 hex digest")
    ca_configured = bool(ca_sha256)

    cloak_version, cloak_url, cloak_sha = _cloakbrowser_values(
        _resolved_text(
            cloakbrowser_version,
            "SATURNX_CLOAKBROWSER_VERSION",
            state,
            state_errors,
            "cloakbrowser_version",
            CLOAKBROWSER_VERSION,
        ...
        _resolved_text(
            cloakbrowser_wheel_url,
            "SATURNX_CLOAKBROWSER_WHEEL_URL",
            state,
            state_errors,
            "cloakbrowser_wheel_url",
            CLOAKBROWSER_WHEEL_URL,
    ...
    disabled = parse_disabled(os.getenv("SATURNX_DISABLED_TOOLS", ""))
    ...
    if not 1 <= configured_rpc_port <= 65_535:
        configured_rpc_port = 15_553
    automatic_text = os.getenv("SATURNX_AUTO_ALLOCATE_PORTS")
    if automatic_text is None:
        automatic_text = read_dotenv_value(
            dotenv_path,
            "SATURNX_AUTO_ALLOCATE_PORTS",
        )
    ...
    environment_values = {
        "SATURNX_INSTALLED_CAPABILITIES": formatted,
        "SKIP_METASPLOIT": "true" if skip_metasploit else "false",
        "SATURNX_BUILD_CA_SHA256": ca_sha256,
        "SATURNX_IMAGE_PLATFORM": selected_platform,
        "SATURNX_CLOAKBROWSER_VERSION": cloak_version,
        "SATURNX_CLOAKBROWSER_WHEEL_URL": cloak_url,
        "SATURNX_CLOAKBROWSER_SHA256": cloak_sha,
        "SATURNX_AUTO_ALLOCATE_PORTS": "true" if automatic_ports else "false",
    ...
    return {
        ...
        "image": {
            ...
            "build_arguments": {
                "SATURNX_CAPABILITIES": formatted,
                "SATURNX_BUILD_FINGERPRINT": fingerprint,
                "SATURNX_BUILD_CA_SHA256": ca_sha256,
                "SATURNX_TARGET_PLATFORM": selected_platform,
                "SATURNX_CAPABILITY_MANIFEST_SHA256": manifest_sha256,
            ...
            "custom_ca": {
                ...
                "buildkit_secret_id": "saturnx_build_ca" if ca_configured else "",
                "maximum_bytes": MAX_BUILD_CA_BYTES,
            },
            "capability_manifest": {
                "path": "/opt/saturnx-capability-manifest.spec",
                "sha256": manifest_sha256,
                "runtime_evidence": "/opt/saturnx-capabilities.txt",
            },
        },
        "cloakbrowser": {
        ...
        "environment": {
            ...
            "operator_paths": ["SATURNX_WORKSPACE_ROOT", "SATURNX_WORDLIST_ROOT"],
        ...
        "runtime": {
            ...
            "port_allocation": {
                ...
                "disable_with": "SATURNX_AUTO_ALLOCATE_PORTS=false",
            },
            "browser_networking": {
...
def setup_information_from_argv(project_root: Path, argv: list[str]) -> dict[str, Any]:
    parser = argparse.ArgumentParser(prog="saturnx --setup-info-json")
