"""
Canonical catalog of SaturnX MCP tools, grouped by the underlying *capability*
(the binary/tool you actually opt out of) and then into operator-facing
categories.

Single source of truth shared by:

  * SaturnX' read-only setup-information surface and selective image metadata;
  * the server ([saturnx/main.py]) — to decide which tools to register, honoring
...
TOOL_REGISTRARS: tuple[ToolRegistrar, ...] = (
    ToolRegistrar("saturnx.tools.network.nmap_tool:register_nmap_tools"),
    ToolRegistrar(
        "saturnx.tools.exploitation.metasploit_tool:register_metasploit_tools",
        metasploit=True,
    ),
    ToolRegistrar("saturnx.tools.exploitation.sqlmap_tool:register_sqlmap_tools"),
    ToolRegistrar("saturnx.tools.web.nuclei_tool:register_nuclei_tools"),
    ToolRegistrar(
        "saturnx.tools.exploitation.searchsploit_tool:register_searchsploit_tools"
    ),
    ToolRegistrar("saturnx.tools.system.shell_tool:register_shell_tools"),
    ToolRegistrar("saturnx.tools.system.file_tool:register_file_tools"),
    ToolRegistrar("saturnx.tools.system.system_tool:register_system_tools"),
    ToolRegistrar("saturnx.tools.recon.recon_tool:register_recon_tools"),
    ToolRegistrar("saturnx.tools.web.web_scanner_tool:register_web_scanner_tools"),
    ToolRegistrar("saturnx.tools.network.network_tool:register_network_tools"),
    ToolRegistrar("saturnx.tools.cracking.cracking_tool:register_cracking_tools"),
    ToolRegistrar("saturnx.tools.ctf.ctf_tool:register_ctf_tools"),
    ToolRegistrar("saturnx.tools.browser.browser_tool:register_browser_tools"),
)
...
CATEGORIES: tuple[ToolCategory, ...] = (
    ToolCategory(
        ...
        capabilities=(
            ...
            Capability("session", "Session / container control", "saturnx session manager",
...
if (FULL_TOOL_COUNT, LIGHT_TOOL_COUNT) != (45, 40):
    raise RuntimeError(
        "SaturnX public tool catalog changed unexpectedly: "
...
def normalize_capabilities(keys: Iterable[str]) -> frozenset[str]:
    ...
    if unknown:
        raise ValueError("unknown SaturnX capabilities: " + ", ".join(sorted(unknown)))
...
def parse_disabled(value: str) -> frozenset[str]:
    """Parse a comma/space separated ``SATURNX_DISABLED_TOOLS`` value into a set
...
def format_disabled(names: set[str] | frozenset[str] | list[str]) -> str:
    """Serialize a disabled-tool set to the ``SATURNX_DISABLED_TOOLS`` form."""
