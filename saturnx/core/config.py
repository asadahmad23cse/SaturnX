"""
SaturnX configuration — loaded from environment variables / .env file.
...
logger = logging.getLogger("saturnx.config")
...
def _parse_disabled_tools(value: str) -> frozenset[str]:
    """Parse SATURNX_DISABLED_TOOLS, filtering out core tools (delegated to the
    tool catalog so the never-disable-core rule lives in exactly one place)."""
    from saturnx.core.tool_catalog import parse_disabled
...
def _parse_docker_network(value: str) -> str:
    ...
    if not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_.-]{0,127}", value):
        raise ValueError("invalid SATURNX_DOCKER_NETWORK name")
    return value
...
def _parse_bind_host(value: str) -> str:
    ...
    if value not in {_EXTERNAL_LISTENER_BIND, "127.0.0.1", "::1"}:
        raise ValueError(
            "SATURNX_LISTENER_BIND_HOST must be 0.0.0.0, 127.0.0.1, or ::1"
        )
    return value
...
class SaturnXConfig:
    """Immutable configuration object for the SaturnX MCP server."""
    ...
    # SATURNX_INSTALLED_CAPABILITIES; this legacy value has no build effect.
    ...
    # SATURNX_DISABLED_TOOLS in .env. Core tools can never be disabled.
    ...
    @classmethod
    def from_env(cls) -> SaturnXConfig:
        ...
        persisted_capabilities = os.getenv("SATURNX_INSTALLED_CAPABILITIES")
        try:
            ...
            raise ValueError("invalid SATURNX_INSTALLED_CAPABILITIES") from exc
        ...
        operator_disabled_tools = _parse_disabled_tools(
            os.getenv("SATURNX_DISABLED_TOOLS", "")
        )
        ...
        if (
            ...
            logger.warning(
                "BROWSER_HEADLESS=false is deprecated and ignored; SaturnX "
        ...
        if legacy_install_mode:
            logger.warning(
                ...
                "use SATURNX_INSTALLED_CAPABILITIES instead."
            )
        return cls(
            ...
            docker_network=_parse_docker_network(
                os.getenv("SATURNX_DOCKER_NETWORK", "")
            ),
            auto_allocate_ports=_parse_bool(
                os.getenv("SATURNX_AUTO_ALLOCATE_PORTS", "true")
            ),
            ...
            wordlist_root=(
                Path(os.environ["SATURNX_WORDLIST_ROOT"]).expanduser().resolve()
                if os.getenv("SATURNX_WORDLIST_ROOT", "").strip()
                else None
            ),
            build_ca_sha256=os.getenv("SATURNX_BUILD_CA_SHA256", "").strip().lower(),
            image_platform=normalize_image_platform(
                os.getenv("SATURNX_IMAGE_PLATFORM")
            ),
            ...
            max_captured_output_bytes=_parse_int(
                os.getenv(
                    "SATURNX_MAX_CAPTURED_OUTPUT_BYTES",
                    str(2 * 1024 * 1024),
                ),
                2 * 1024 * 1024,
                minimum=64 * 1024,
            ...
            max_inline_response_chars=_parse_int(
                os.getenv("SATURNX_MAX_INLINE_RESPONSE_CHARS", "12000"),
                12_000,
                minimum=256,
                maximum=1_000_000,
            ),
            max_inline_file_bytes=_parse_int(
                os.getenv("SATURNX_MAX_INLINE_FILE_BYTES", str(8 * 1024 * 1024)),
                8 * 1024 * 1024,
                minimum=64 * 1024,
                maximum=512 * 1024 * 1024,
            ),
            max_background_jobs=_parse_int(
                os.getenv("SATURNX_MAX_BACKGROUND_JOBS", "8"),
                8,
                minimum=1,
                maximum=128,
            ),
            ...
            workspace_root=(
                Path(os.environ["SATURNX_WORKSPACE_ROOT"]).expanduser()
                if os.getenv("SATURNX_WORKSPACE_ROOT", "").strip()
                else None
            ),
            workspace_auto_prune=_parse_bool(
                os.getenv("SATURNX_WORKSPACE_AUTO_PRUNE", "false")
            ),
            workspace_retention_days=_parse_int(
                os.getenv("SATURNX_WORKSPACE_RETENTION_DAYS", "0"),
                0,
                minimum=0,
                maximum=365_000,
            ),
            workspace_max_sessions=_parse_int(
                os.getenv("SATURNX_WORKSPACE_MAX_SESSIONS", "0"),
                0,
                minimum=0,
                maximum=1_000_000,
            ),
            workspace_max_bytes=_parse_int(
                os.getenv("SATURNX_WORKSPACE_MAX_BYTES", "0"),
                0,
                minimum=0,
            ),
            listener_ports=_parse_ports(
                os.getenv("SATURNX_LISTENER_PORTS", "4444-4464")
            ),
            listener_bind_host=_parse_bind_host(
                os.getenv("SATURNX_LISTENER_BIND_HOST", _EXTERNAL_LISTENER_BIND)
            ),
        )
