"""
Stealth browser automation tools for SaturnX MCP.
...
from saturnx.core.guidance import (
    TOOL_DESCRIPTIONS,
    backend_unavailable,
    missing_param_error,
    path_error,
...
from saturnx.core.security import (
    redact_secrets,
    redact_url,
...
from saturnx.output.sanitizer import escape_display_controls

if TYPE_CHECKING:
    from fastmcp import FastMCP

logger = logging.getLogger("saturnx.tools.browser")
...
async def _resolve_cloak(docker) -> str:
    ...
    unavailable. SaturnX does not silently use agent-browser's fallback.
    """
...
async def _backend(ctx: Context | None, tool: str, *, launch_env: dict | None = None):
    ...
    if not cloak:
        return backend_unavailable(
            tool,
            "The cloakbrowser Chromium binary is unavailable; SaturnX will not "
    ...
    async def browser_cmd(
        ...
        if re.search(r"(?:^|\s)--headed(?:=\S*)?(?=\s|$)", args):
            return {
                ...
                "option": "--headed",
                "error": (
                    "SaturnX browser sessions are headless-only. Remove --headed; "
